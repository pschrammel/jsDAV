    ...
    this.invoke = async function() {
        ...
        //     // Make sure this is a HTTP method we support
            if (internalMethods[method]) {
              await self["http" + method.charAt(0) + method.toLowerCase().substr(1)]();
            } else {
...
//        Util.log("httpPropfind BEFORE getPropertiesForPath '" + path + "';", requestedProperties, depth);
    ...
    this.httpMove = async function() {
        ...
        await this.server.tree.move(moveInfo)
        this.markDirty(moveInfo.source);
        this.markDirty(moveInfo.destination);
    ...
    this.httpCopy = async function() {
        ...
        await this.server.tree.copy(copyInfo)
        this.markDirty(copyInfo.destination);
...
//            }
    ...
    this.updateProperties = async function(uri, properties) {
        ...
        if (!hasError) {
            var updateResult = await node.updateProperties(properties)
            remainingProperties = {};
