...
(function() {
    ...
    this.invoke = function() {
        ...
        //     // Make sure this is a HTTP method we support
            if (internalMethods[method]) {
                var res = self["http" + method.charAt(0) + method.toLowerCase().substr(1)]();
                if (res && res.then) {
                    ...
                    res.then(function(done){
                        ...
                        console.log("Ups resolution failed",err)
                        throw(err)
                    })
                }
            } else {
    ...
    // TBD not founds are not cached!
    this.asyncGetNodeForPath = async function(path, noThrowError) {
        var node;
        if (this.nodeCache[path]) {
            // TBD: nil might be cached but might not always be returned
            return Promise.resolve(this.nodeCache[path]);
        }

        try {
            node = await this.server.tree.getNodeForPath(path)
        } catch(err) {
            if (!noThrowError) {
                throw(err)
            }
    ...
    this.httpOptions = async function() {
        var uri  = this.getRequestUri();
        var methods = await this.getAllowedMethods(uri)
    ...
    this.httpGet = async function() {
        ...
        var redirected = await this.asyncCheckPreconditions(node, true)
        if (redirected)
            return;
        if (!node)
        ...
        console.log("getting headers");
            ...
            if (!ignoreRangeHeader && nodeSize && range) {
                ...
                await this.upstream(node,start,end,httpHeaders)
            } else {
    ...
    this.upstream = async function(node,start,end,httpHeaders) {
        var writeStreamingHeader = function () {
            self.httpResponse.writeHead(206, httpHeaders);
        };
        var self = this;
        var stream = await node.getStream(start, end)
        ...
        stream.on("data", function(data) {
            ...
            if (self.httpResponse.write(data)) {
                console.log("writing2",data.length)
            } else {
                //                                stream.pause()
                console.log("paused", data.length);
...
//        Util.log("httpPropfind BEFORE getPropertiesForPath '" + path + "';", requestedProperties, depth);
        var newProperties= await this.asyncGetPropertiesForPath(path, requestedProperties, depth)
//        Util.log("httpPropfind AFTER getPropertiesForPath '" + path + "';",  newProperties);
    ...
    this.httpPut = async function() {
        ...
        if (!node) {
            ...
            await this.getStreamBody(stream);
            this.markDirty(uri);
            ...
            var redirected = await this.asyncCheckPreconditions(node,false)
            if (redirected)
                return false;
     ...
     * @return {Array}
     */
    this.getAllowedMethods = async function(uri) {
        var self   = this;
        var methods = [
            "OPTIONS",
            "GET",
        ...
        return   Util.makeUnique(methods)
    };
    ...
    this.getStreamBody = async function(stream) {
        var req = this.httpRequest;
        var contentLength = req.headers["content-length"];
    ...
    this.asyncCheckPreconditions = async function(node,handleAsGET) {
        handleAsGET = handleAsGET || false;
        var uri, ifMatch, ifNoneMatch, ifModifiedSince, ifUnmodifiedSince;
        var lastMod = null;
        var etag    = null;
        ...
        if (!ifNoneMatch && (ifModifiedSince = self.httpRequest.headers["if-modified-since"])) {
            ...
            // If-None-Match header as per the HTTP spec.
            var date = new Date(ifModifiedSince);
            lastMod = await node.getLastModified()
            if (lastMod) {
                lastMod = new Date("@" + lastMod);
                if (lastMod <= date) {
                    self.httpResponse.writeHead(304);
    ...
    this.getPropertiesForPath = function(path, propertyNames, depth, cbgetpropspath) {
        ...
        this.getNodeForPath(path, function(err, parentNode) {
            ...
            //if (jsDAV.debugMode)
            //    Util.log("getPropertiesForPath", depth, parentNode,parentNode.hasFeature(jsDAV_iCollection));
    ...
    this.asyncGetPropertiesForPath = async function(path, propertyNames, depth) {
        ...
        if (depth === 1 && parentNode.hasFeature(jsDAV_iCollection)) {
            var cNodes = await parentNode.getChildren()
            for (var i = 0, l = cNodes.length; i < l; ++i) {
                nodes[path + "/" + cNodes[i].getName()] = cNodes[i];
                nodesPath.push(path + "/" + cNodes[i].getName());
            }
...
/*        console.log("allProperties",allProperties, propertyNames,parentNode,
                    parentNode.UUID,
                    parentNode.UUIDS,
                    jsDAV_iCollection.UUID,
                    jsDAV_iFile.UUID,
                    jsDAV_iProperties.UUID,
                    parentNode.hasFeature(jsDAV_iFile),
                    parentNode.hasFeature(jsDAV_iCollection),
                    cNodes, depth)
*/
        ...
        nodesPath.forEach(function(myPath) {
            var node = nodes[myPath];
            var newProperties = {
                "200" : {},
                "403" : {},
            ...
            myProperties = Object.keys(propertyMap);
            if (node.hasFeature(jsDAV_iProperties)) {
                props=node.getProperties(myProperties)
                // The getProperties method may give us too much,
                // properties, in case the implementor was lazy.
                //
                // So as we loop through this list, we will only take the
    ...
    this.parseProppatchRequest = function(body, cbproppatchreq) {
        ...
        var self = this;
        Xml.loadDOMDocument(body, this.server.options.parser, function(err, dom) {
            if (!Util.empty(err))
            ...
            for (; i < l; ++i) {
                ...
                innerProperties = Xml.parseProperties(child, self.propertyMap);
    ...
    this.updateProperties = function(uri, properties, cbupdateprops) {
        ...
        this.getNodeForPath(uri, function(err, node) {
            ...
            function afterUpdateProperties() {
    ...
    this.createFile = function(uri, dataOrStream, enc, cbcreatefile) {
        ...
        this.dispatchEvent("beforeBind", uri, function(stop, etag) {
            if (stop)
                return cbcreatefile(stop === true ? null : stop, etag);
            ...
            self.getNodeForPath(dir, function(err, parent) {
                ...
                self.dispatchEvent("beforeCreateFile", uri, dataOrStream, enc, parent, function(stop, etag) {
                    ...
                    if (dataOrStream === null) {
                        ...
                        if (parent.createFileStreamRaw && dataOrStream instanceof Fs.ReadStream)
                            parent.createFileStreamRaw(name, dataOrStream, enc || "utf8", afterBind);
                        else
                            parent.createFile(name, dataOrStream, enc || "utf8", afterBind);
                    }

                    function afterBind(err, etag) {
                        if (!Util.empty(err))
                            return cbcreatefile(err);

                        var path = Path.join(dir, name);
                        self.markDirty(path);
                        self.dispatchEvent("afterBind", path);
                        cbcreatefile(null, etag);
                    }
                });
            });
        });
    ...
    this.createCollection = async function(uri, resourceType, properties) {
        ...
        if (parent.hasFeature(jsDAV_iExtendedCollection)) {
            await parent.createExtendedCollection(newName, resourceType, properties);
        } else {
            ...
            if (properties.length > 0) {
                try {
                    var errorResult = self.updateProperties(uri, properties)
                ...
                function rollback(exc) {
                    var node = self.server.tree.getNodeForPath(uri)
                    node["delete"]();
                    throw(err)
                }
            }
