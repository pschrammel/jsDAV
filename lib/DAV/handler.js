...
(function() {
    ...
    this.asyncGetNodeForPath = async function(path) {
        if (this.nodeCache[path])
            return Promise.resolve(this.nodeCache[path]);

        var node = await this.server.tree.getNodeForPath(path)
    ...
    this.httpGet = async function() {
        ...
        function afterCheck() {
            ...
            self.getHTTPHeaders(uri, async function(err, httpHeaders) {
                if (!Util.empty(err))
                    return self.handleError(err);
                var nodeSize = null;
...
//                                stream.pause()
    ...
    this.asyncCheckPreconditions = async function(handleAsGET) {
        handleAsGET = handleAsGET || false;
        var uri, ifMatch, ifNoneMatch, ifModifiedSince, ifUnmodifiedSince;
        var node    = null;
        var lastMod = null;
        ...
        if (ifMatch = this.httpRequest.headers["if-match"]) {
            ...
            // request succeed if a resource exists at that url.
            try {
                node = await this.asyncGetNodeForPath(uri)
            } catch(err) {
