...
var jsDAV_Auth_Backend_AbstractBasic = module.exports = jsDAV_Auth_iBackend.extend({
     ...
     * @return bool
     */
    validateUserPass: async function(username, password) {},
     ...
     * @return void
     */
    requireAuth: function(realm, err) {
     ...
     * @return bool
     */
    authenticate: async function(handler, realm) {
...
//        console.log("headers", req.headers)
        if (!auth || auth.toLowerCase().indexOf("basic") !== 0)
            this.requireAuth(realm, "No basic authentication headers were found");

        var userpass = Buffer.from(auth.substr(6), "base64").toString("utf8").split(":");
        if (!userpass.length)
            this.requireAuth(realm, "No basic authentication headers were found");
//        console.log("authenticating", userpass, auth.substr(6))
        this.currentUser= await this.validateUserPass(userpass[0], userpass[1])
