...
exports.toClarkNotation = function(dom) {
    ...
    if (dom.nodeType !== 1 || !dom.localName)
        return null;
//    console.log(">>>>>>>>>>", dom.namespaceURI,dom.nodeType, dom.localName)
    if (dom.namespaceURI==="")
        throw(new Exc.BadRequest(`invalid namespace '${dom.namespaceURI}','${dom.nodeType}'`))
 ...
 * @return DOMDocument
 */
exports.loadDOMDocument = function(xml, which) {
    if (!xml)
        throw(new Exc.BadRequest("Empty XML document sent"));
    ...
    try {
        ...
        throw(new Exc.BadRequest("The request body had an invalid XML body. (message: " +
        ex.message + ")"));
    }
